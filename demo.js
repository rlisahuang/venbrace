...
if (lexer.getErrors().length === 0) {
    ...
    if (dom === undefined) {
        ...
        document.getElementById("output").innerHTML = "Translating failed. Please revise your input.";
    } else {
        valueTokens = tokens;
        if (stringForParserTokens) editor.setValue(stringForParserTokens);
        ...
        if (!parsingError) {
        console.log("correct!");
        document.getElementById("output").innerHTML = "Translation accepted.";
        } else {
...
for (var i=0; i<tokens.length; i++) {
    ...
    ////console.log(t);
    if (t.type == tokenIntMap['NEG_NUM'] && i > 0) { // t is NEG_NUM
    var prev = tokens[i-1];
    if (prev.type == tokenIntMap['NUMBER']
        || prev.type == tokenIntMap['NEG_NUM']
        || prev.type == tokenIntMap['ID']
        || prev.type == tokenIntMap['RPAREN']) { // prev is NUM, NEG_NUM, ID, or RPAREN (end of an expr)

        var op = t.clone();
        op.text = "-";
        op.type = tokenIntMap['MINUS'];
        op.stop = op.start;

        t.text = t.text.substring(1);
        t.type = tokenIntMap['NUMBER'];
        t.start = t.start + 1;
        t.column = t.column + 1;
